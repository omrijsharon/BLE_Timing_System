...
void setLedColor(String address, int rssi) {
  String addressWithColons;

  // Check the length of the address and add colons if needed
  if (address.length() == 12) {
    addressWithColons = addColonsToMacAddress(address);
  } else if (address.length() == 17) {
    addressWithColons = address;
  } else {
    // Invalid MAC address length, return without setting LED color
    return;
  }
  // Convert MAC address to integer
  uint8_t macInt = processMacAddress(addressWithColons);
